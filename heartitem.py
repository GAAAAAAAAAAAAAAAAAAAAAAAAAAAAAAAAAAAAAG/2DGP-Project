...
class Heartitem:
    ...
    def __init__(self, x = None, y = None):
        if Heartitem.image == None:
            Heartitem.image = load_image('heart.png')
    ...
    def handle_collision(self, group, other):
        match group:
            case 'player:heartitem':
                # fill here
                game_world.remove_object(self)
                h = Heart(450 + (2 - server.player.hp) * 55)
                #server.hearts.append(h)
                server.hearts.insert(0, h)
                game_world.add_object(h, 2)
                server.player.hp += 1
                pass